...
def load_locked_graph(name, lock_path):
    """Fetch a pinned parameterized graph named by a template's workflow.lock.json.
    ...
    cache = lock_path.parent / '_toolkit_graphs' / entry['path'].split('/')[-1]
...
def main(argv=None):
    ...
    parser.add_argument('--workflow-lock', type=Path,
                        help='Optional template workflow.lock.json; when given, its pinned graphs are checked too')
    ...
    graph_results = {}
    if args.workflow_lock:
        for name in ('first', 'continue'):
            graph = load_locked_graph(name, args.workflow_lock)
        ...
        graph_results['skipped'] = 'no --workflow-lock supplied: environment checks only'
        pending.append('No --workflow-lock: the template pinned graphs were not checked against this instance.')
