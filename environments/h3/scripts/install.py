...
def main(argv=None):
    ...
    lines = ['#!/usr/bin/env bash', 'set -euo pipefail',
             ...
             'ENVIRONMENT=' + q(str(root)), 'DEST=' + q(str(target)),
             ...
             q(a.python) + ' -c ' + q("import pathlib,sys; (pathlib.Path(sys.argv[1])/'.h3-isolated-install').write_text('h3-new-environment-v1\n')") + ' "$DEST"',
    ...
    lines.extend([
        '"$DEST/.venv/bin/python" -m pip install --only-binary=:all: --no-deps --require-hashes -r "$ENVIRONMENT/vendor/python/requirements-direct-linux-py310.lock.txt"',
        ...
        '# Next: README.md in this environment directory, then model download with explicit license acceptance.',
