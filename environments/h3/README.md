...
这是一份**记录**，不是前置条件。它写下作者实际跑通 MiniMax H3 时用的版本组合：ComfyUI Core、H3 自定义节点、Python/CUDA 依赖闭包、模型权重、兼容补丁。

## 默认怎么用：先跑用户自己的环境

- 用户机器上已有 ComfyUI 和模型时，**直接用他那套推理**。不要求对齐本记录的版本，不必先重装，也不要把这里当成推理前必须通过的检查。
- 只有遇到报错、节点缺失、版本不匹配，需要判断「到底该换成什么版本、缺哪个补丁」时，再来查这里。按需取用其中一段——某个版本号、依赖锁里某个 wheel、某一个补丁——而不是整套照搬。
- 环境是可以逐步修的：先看清报错指向哪一层（缺节点 / 缺模型 / wheel 冲突 / 源码补丁），只动那一层。

第 1–9 节同时构成一套完整的独立安装流程，用于确实没有可用环境、或使用者明确要求另建一套的情形。

**验收边界：安装计划、源码补丁和 CPU 侧测试已检查，完整干净 Linux 安装与 GPU 推理未验。任何「装成功」的结论都只指命令返回成功，不代表这套续接能跑。**

## 0. 要建立独立环境时，绝不改动用户已有的环境

**为跑一个模板改坏环境，往往会连带打断用户其他的 ComfyUI 任务**，所以这一节是硬约束：
...
- 需要复用时，让其他模板引用同一个 `--target`，而不是再装一套。
